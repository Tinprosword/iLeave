...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="row" style="padding-bottom:0px;margin-top:1px; height:500px;overflow-y:scroll" id="maindata">
        <asp:Repeater ID="rp_list" runat="server">
            <ItemTemplate>
                ...
                <div class="col-xs-12 divheighter"><asp:Label ID="lb_name" runat="server"><%#GetStaffName((WebServiceLayer.WebReference_leave.LeaveRequestMaster)Container.DataItem)%></asp:Label><div style="float:right"><asp:Label ID="lb_status" runat="server"><%#GetLeaveStatus((WebServiceLayer.WebReference_leave.LeaveRequestMaster)Container.DataItem)%></asp:Label>&nbsp;<img alt="Detail" src="../Res/images/details.png" style="width:24px; height:24px;  margin-right:10px;" onclick="SingleResult('../webservices/leave.asmx/GetLeaveDetail_html',{requestID:<%#((WebServiceLayer.WebReference_leave.LeaveRequestMaster)Container.DataItem).RequestID%>,leaveid:<%#((WebServiceLayer.WebReference_leave.LeaveRequestMaster)Container.DataItem).MinLeaveID%>,staff:<%#(int)loginer.userInfo.staffid %>,employmentNo:<%#(int)loginer.userInfo.employID%>,lan:<%#(int)BLL.MultiLanguageHelper.GetChoose()%>},'string',onGetData)"/></div></div>
