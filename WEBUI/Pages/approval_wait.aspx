            ...
            <ItemTemplate>
                ...
                <div class="col-xs-12 divheighter">
                    ...
                    <div style="float:right">
                        <asp:Label ID="lb_status" runat="server"><%#GetLeaveStatus((WebServiceLayer.WebReference_leave.LeaveRequestMaster)Container.DataItem)%></asp:Label> &nbsp; <a style="cursor:pointer;color:White;background-color:#2573A4;font-size:16px;height:34px; padding:4px;" onclick="SingleResult('../webservices/leave.asmx/GetLeaveDetail_html',{requestID:<%#((WebServiceLayer.WebReference_leave.LeaveRequestMaster)Container.DataItem).RequestID%>,leaveid:<%#((WebServiceLayer.WebReference_leave.LeaveRequestMaster)Container.DataItem).MinLeaveID%>,staff:<%#(int)loginer.userInfo.staffid %>,employmentNo:<%#(int)loginer.userInfo.employID%>,lan:<%#(int)BLL.MultiLanguageHelper.GetChoose()%>},'string',onGetData)"><%=BLL.MultiLanguageHelper.GetLanguagePacket().application_detail_link_detail %></a>
                    </div>
                </div>
