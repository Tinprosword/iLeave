        ...
        <div class="col-xs-12 lsf-clearPadding" style="height:400px; overflow:scroll;padding-left:5px;padding-right:0px">
            <table class="col-xs-12 lsu-table-xs" style="font-size:15px">
                <asp:Repeater ID="repeater_myapplications" runat="server">
                    <ItemTemplate>
                        <tr style="height:42px"><td class="col-xs-4" style="padding:0px"><%# ((BLL.workflow.Worktask_leave)Container.DataItem).leaveRequestMaster.uname %></td><td class="col-xs-7" style="padding:0px"><%# ((BLL.workflow.Worktask_leave)Container.DataItem).leaveRequestMaster.leavefrom.ToString("MM-dd")%>-><%# ((BLL.workflow.Worktask_leave)Container.DataItem).leaveRequestMaster.leaveto.ToString("MM-dd")%>(<%# ((BLL.workflow.Worktask_leave)Container.DataItem).leaveRequestMaster.totaldays %>)</td><td class="col-xs-1" style="padding:0px"><asp:LinkButton ID="lb" runat="server" CommandArgument="<%# ((BLL.workflow.Worktask_leave)Container.DataItem).worktask.taskDetailID %>" style="font-size:24px">></asp:LinkButton></td></tr>
                    </ItemTemplate>
                    <AlternatingItemTemplate>
                        <tr style="height:42px; background-color:aliceblue"><td class="col-xs-4" style="padding:0px"><%# ((BLL.workflow.Worktask_leave)Container.DataItem).leaveRequestMaster.uname %></td><td class="col-xs-7" style="padding:0px"><%# ((BLL.workflow.Worktask_leave)Container.DataItem).leaveRequestMaster.leavefrom.ToString("MM-dd")%>-><%# ((BLL.workflow.Worktask_leave)Container.DataItem).leaveRequestMaster.leaveto.ToString("MM-dd")%>(<%# ((BLL.workflow.Worktask_leave)Container.DataItem).leaveRequestMaster.totaldays %>)</td><td class="col-xs-1" style="padding:0px"><asp:LinkButton ID="lb" runat="server" CommandArgument="<%# ((BLL.workflow.Worktask_leave)Container.DataItem).worktask.taskDetailID %>" style="font-size:24px">></asp:LinkButton></td></tr>
                    </AlternatingItemTemplate>
                </asp:Repeater>
            </table>
        </div>
